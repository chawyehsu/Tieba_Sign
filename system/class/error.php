...
class error
{
	static public function system_error($message, $show = true, $halt = true)
	{
	...
	static public function db_error($message, $sql)
	{
		global $_G;
		list($showtrace, $logtrace) = error::debug_backtrace();
		$db = &DB::object();
				...
				if (!empty($error["args"])) {
					...
					foreach ($error["args"] as $arg ) {
						...
						if (is_array($arg)) {
							...
							$fun .= (defined("DEBUG_FLAG") && DEBUG_FLAG ? "'" . htmlspecialchars(substr(self::clear($arg), 0, 10)) . (10 < strlen($arg) ? " ..." : "") . "'" : "%s");
						}
	...
	static public function clear($message)
	{
		return str_replace(array("\t", "\r", "\n"), " ", $message);
	}

	static public function sql_clear($message)
	{
