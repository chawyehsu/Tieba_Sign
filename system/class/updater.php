...
class Updater
{
	...
	static public function write_file()
	{
		...
		if (!$files) {
			return array("status" => -255);
		}

		foreach ($files as $file ) {
			if (!self::_is_writable(ROOT . $file["path"])) {
				$err_file[] = $file["path"];
			}
		}
		...
		foreach ($files as $file ) {
			...
			if (md5_file(ROOT . $file["path"]) != md5($file["content"])) {
				return array("status" => -2, "file" => $file["path"]);
			}
		}
	...
	static private function _write($path, $content)
	{
		$fp = @fopen($path, "wb");
	...
	static private function _is_writable($path)
	{
		if (!file_exists($path)) {
	...
	static private function _download_file($path, $hash, $try = 1)
	{
