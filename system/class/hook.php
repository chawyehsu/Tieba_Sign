...
class HOOK
{
				...
				if (property_exists($_PLUGIN["obj"][$pluginid], "version")) {
					...
					if ($plugin["ver"] != $version) {
						...
						if (method_exists($_PLUGIN["obj"][$pluginid], "on_upgrade")) {
							$_PLUGIN["obj"][$pluginid]->on_upgrade($plugin["ver"]);
						}
					}
				}
	...
	public function parse_module($module, $pluginid)
	{
		global $_PLUGIN;

		switch ($module["type"]) {
		case "page":
			$_PLUGIN["page"][] = array("id" => "$pluginid-$module[id]", "title" => $module["title"], "file" => ROOT . "./plugins/$pluginid/" . $module["file"], "admin" => $module["admin"]);
			break;
