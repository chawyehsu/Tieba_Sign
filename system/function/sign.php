...
function _update_liked_tieba($uid, $ignore_error = false, $allow_deletion = true)
{
	...
	foreach ($liked_tieba as $tieba ) {
		if ($my_tieba[$tieba["name"]]) {
			...
			if (!$my_tieba[$tieba["name"]]["fid"]) {
				DB::update("my_tieba", array("fid" => $tieba["fid"]), array("uid" => $uid, "name" => $tieba["name"]), true);
			}

			continue;
		}
		else {
			DB::insert("my_tieba", array("uid" => $uid, "fid" => $tieba["fid"], "name" => $tieba["name"], "unicode_name" => $tieba["uname"]), false, true, true);
			$insert++;
		}
	}
