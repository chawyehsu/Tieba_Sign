...
if (($_GET["action"] == "logout") && ($_GET["hash"] == $formhash)) {
	...
	if ($uid && ($_GET["action"] == "unbind_user")) {
		if ($_GET["formhash"] != $formhash) {
			showmessage("来源不可信，请重试", "./");
		}
		...
		if ($uid && ($_GET["action"] == "bind_user")) {
			...
			if ($user) {
				...
				if ($exists) {
					showmessage("您此前已经绑定过此帐号", "./#");
				}

				DB::insert("member_bind", array("uid" => $uid, "_uid" => $userid, "username" => $user["username"]));
				...
				if (!$exists) {
					...
					DB::insert("member_bind", array("uid" => $userid, "_uid" => $uid, "username" => $username));
				}
			...
			if ($uid && ($_GET["action"] == "switch")) {
			...
			else if ($_GET["action"] == "find_password") {
				if ($_GET["token"]) {
				...
				if ($_POST && strexists($_SERVER["HTTP_REFERER"], "member.php")) {
					...
					if ((getsetting("register_check") && ((TIMESTAMP - 5) < $time)) || ($time < (TIMESTAMP - 300))) {
						$_POST = array();
					}

					if (getsetting("register_limit") && ($member_count != $count)) {
						showmessage("当前注册人数过多，请您稍后再试", "member.php");
					}

					if (1000 < $count) {
						showmessage("超过当前站点最大用户数量上限，无法注册", "member.php");
					}
					...
					if (!$_POST["username"]) {
						...
						if ($invite_code && ($_POST["invite_code"] != $invite_code)) {
							showmessage("邀请码有误", "member.php");
						}
						...
						if (!$username || !$password || !$email) {
							showmessage("您输入的信息不完整", "member.php");
						}

						if (preg_match("/[<>'\\\"]/i", $username)) {
							showmessage("用户名中有被禁止使用的关键字", "member.php");
						}
