...
case "update_liked_tieba":
	...
	showmessage("喜欢的贴吧列表已经更新,<br>新增$insert个贴吧, 删除$deleted个贴吧", "admin.php#user", 1);
	break;
...
case "mail_advanced":
	...
	foreach ($_config as $k => $v ) {
		$key = $v[1];
		$item = array("key" => $v[1], "name" => $v[0], "description" => $v[2], "value" => isset($setting[$key]) ? $setting[$key] : $v[3], "type" => $v[4] ? $v[4] : "text");
		$out[] = $item;
	}
