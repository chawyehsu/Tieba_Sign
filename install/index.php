...
case "check":
	...
	$content .= "</tbody></table>";
	if (ini_get("allow_url_fopen") && function_exists("curl_init") && function_exists("fsockopen") && is_writable($config_file)) {
...
case "install":
	...
	if (!$link) {
		show_back("数据库配置", "错误：无法连接数据库服务器！</p><p>" . mysql_error());
	}
	...
	if (!$selected) {
		...
		if (!$selected) {
			show_back("数据库配置", "错误：指定的数据库不可用</p><p>" . mysql_error());
		}
	}
	...
	if ($err) {
		show_back("正在安装", "安装过程出现错误:</p><p>" . mysql_error());
	}

	$_config = array(
		"version" => $version,
		"db"      => array("server" => $db_host, "port" => $db_port, "username" => $db_username, "password" => $db_password, "name" => $db_name, "pconnect" => $db_pconnect)
		);
